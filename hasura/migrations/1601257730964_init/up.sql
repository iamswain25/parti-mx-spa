...
COMMENT ON COLUMN mx.groups.bg_img_url IS '데스크탑 크기 그룹 이미지';
COMMENT ON COLUMN mx.groups.mb_img_url IS '모바일 크기 그룹 이미지';
CREATE FUNCTION mx.get_group_by_post_id(post_id integer) RETURNS SETOF mx.groups
    LANGUAGE sql STABLE
    AS $$
select
  mx.groups.*
from
  mx.posts
  inner join mx.boards on mx.boards.id = mx.posts.board_id
  inner join mx.groups on mx.groups.id = mx.boards.group_id
where
  mx.posts.id = post_id;
$$;
...
CREATE FUNCTION mx.get_new_post_count(groupid integer, userid integer) RETURNS SETOF mx.board_new_post_count
    LANGUAGE sql STABLE
    AS $$
select 
...
CREATE FUNCTION mx.user_like_count_in_post(posts_row mx.posts, hasura_session json) RETURNS integer
    LANGUAGE sql STABLE
    AS $$
